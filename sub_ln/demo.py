...
FORMAT = "[%(asctime)s - %(levelname)8s - %(filename)22s - %(funcName)10s() ] - %(message)s"
...
order_params = {'message': message['message'],
                'bid': 10000,
...
if refund_address.status_code == 200:
    ...
    logging.debug(f"Successfully got refund address {refund_address['address']} for the swap")
else:
    logger.debug(f"Failed to get a refund address")

time.sleep(2)
# create the swap with swap server
create_swap_params = {'uuid': uuid,
...
while tries < 10:
    ...
    if 'payment_secret' in swap_status['swap_check']:
        logger.debug(f"Payment complete! Preimage: {swap_status['swap_check']['payment_secret']}")
        complete = True
        break
